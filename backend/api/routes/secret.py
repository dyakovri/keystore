...
from api.models.db import Secret, SecretOwners, Access
...
async def get_secrets(auth = Depends(UnionAuth(scopes=[]))) -> list[SecretGet]:
    secrets = Secret.query(session=db.session).join(SecretOwners).filter(SecretOwners.owner_id == auth["id"]).one_or_none()
